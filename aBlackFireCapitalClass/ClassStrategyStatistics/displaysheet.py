...
class DisplaysheetStatistics:
	...
	def get_results(self):
		...
		stats['price'] = cstat.cum_returns(stats['return'])

		# Rolling Annualised Sharpe
		rolling = stats['return'].rolling(window=self.rolling_periods)
		rolling_sharpe_s = np.sqrt(self.rolling_periods) * (
		...
		# Equity statistics
		stats["sharpe"] = cstat.create_sharpe_ratio(
            stats['return'], self.rolling_periods
        )
		...
		if self.benchmark is not None:
			portfolio = pd.merge(stats['return'], self.benchmark, left_index=True, right_index=True)
			stats['beta'] = cstat.strategy_beta(portfolio)
			stats['return_b'] = portfolio[['benchmark']]
			stats['price_b'] = cstat.cum_returns(portfolio[['benchmark']], of='benchmark')

			stats["sharpe_b"] = cstat.create_sharpe_ratio(portfolio[['benchmark']], self.rolling_periods)
			...
			rolling = portfolio['benchmark'].rolling(window=self.rolling_periods)
			rolling_sharpe_s = np.sqrt(self.rolling_periods) * (
				rolling.mean() / rolling.std()
			)
	...
	def _plot_txt_curve(self, stats, ax=None, **kwargs):
		...
		sharpe = cstat.create_sharpe_ratio(returns, self.rolling_periods)
		...
		ax.text(7.50, 5.9, '{:.2%}'.format((returns.std() * np.sqrt(self.rolling_periods)).values[0]), fontweight='bold', horizontalalignment='right', fontsize=8)
		...
		ax.text(7.50, 3.9, '{:.2%}'.format(dd_max), color='red', fontweight='bold', horizontalalignment='right', fontsize=8)
		...
		ax.text(9.75, 1.9, '{:.2f}'.format(stats['turnover']), fontweight='bold', horizontalalignment='right', fontsize=8)
		...
		if self.benchmark is not None:
			...
			sharpe_b = cstat.create_sharpe_ratio(returns_b, self.rolling_periods)
			...
			ax.text(9.75, 5.9, '{:.2%}'.format((returns_b.std() * np.sqrt(self.rolling_periods)).values[0]), fontweight='bold', horizontalalignment='right', fontsize=8)
