...
def shift_mc(group, period):
    """
        This function is used to shift the market for n period in a group of Data.
        This shift will be further used to compute the monthly return of the sectors in a month.

        :param group: Data frame of containing the monthly prices informations
        :param periode: shift periode in month
    ...
    t = group[['mc']].shift(periods=period, freq='M')
    group.loc[:, 'mc'] = t['mc']
...
def calculate_sectors_summary(group) -> pd.DataFrame:
    ...
    try:
        ret = (group['mc_s'] * group['return']).sum() / group['mc_s'].sum()
    except:
        ret = None
    pt_return = (group['mc_s'] * group['pt_return']).sum() / group['mc_s'].sum()
    ...
    tab.columns = ['mc', 'vol', 'npt', 'npptvar', 'nptvar', 'nrc', 'nrcvar', 'return', 'pt_return', 'mpt_return',
                   'pptvar', 'ptvar', 'rc', 'rcvar']
...
#######################################################################################################################
#
# Merge Stocks Data to the corresponding naics, eco zone
#
#######################################################################################################################


def get_list_of_eco_zone_and_naics(connection_string) -> pd.DataFrame:
    """
    This function is used to map all the naics and the eco zone for each economic area
    :param connection_string: connection to the Mongo database
    :return: data frame containing the mapping between economics zone and naics levels 1  and 2.
    """
    # Open the connection with the mongo db
    client_db = motor.motor_tornado.MotorClient(connection_string)
    loop = tornado.ioloop.IOLoop

    # get the eco zone data
    zone_eco_tab = loop.current().run_sync(
        EconomicsZonesDataInfos(client_db, {}, {"eco zone": 1}).GetEconomicsZonesFromDB)

    # get the naics for level 1 and 2
    naics_tab = tornado.ioloop.IOLoop.current(). \
        run_sync(SectorsMarketDataInfos(
        client_db, {"$or": [{"level": "1"}, {"level": "2"}]}, {"_id": 1, "level": 1}).GetDataFromDB)

    # Create a DataFrame that map the naics and the eco zone
    t = pd.DataFrame([[zone['eco zone'], naics['_id'], naics['level']] for zone in zone_eco_tab for naics in naics_tab],
    ...
    d['header'] = t.columns
    d['data'] = data
    d['description'] = 'This table contains a mapping between naics and eco zone. Refer to function ' \
                       'get_list_of_eco_zone_and_naics in BlackFire_Capital/bBlackFireCapitalData/SectorsMarketData/' \
                       'GetSectorsMarketData.py function: get_list_of_eco_zone_and_naics(connection_string).'
    np.save('ZoneAndNaics', d)
...
def get_monthly_stocks_summary_from_the_db(params):
    ...
    tab_of_stocks_price = loop.current().run_sync(
        StocksMarketDataPrice(ClientDB, params.value, pipeline).SetIndexCreation)

    # Get Last Stocks Price of the month from the MongoDB
    loop = tornado.ioloop.IOLoop
    tab_of_stocks_price = loop.current().run_sync(
        StocksMarketDataPrice(ClientDB, params.value, pipeline).GetMontlyPrice)
...
def get_monthly_sectors_summary(params) -> pd.DataFrame:
    ...
    data['mc'] = data['csho'] * data['pc']
    data['mc_s'] = data[['date', 'naics', 'isin', 'mc']].groupby(['naics', 'isin']).apply(shift_mc, 1)

    # Compute summary informations
    __HEADER__ = ['date', 'naics', 'eco zone', 'mc', 'vol', 'npt', 'npptvar', 'nptvar', 'nrc', 'nrcvar', 'pt_return',
                  'pptvar', 'ptvar', 'return', 'rcvar', 'rc', 'mc_s']

    result = data[__HEADER__].groupby(['eco zone', 'naics', 'date']).apply(calculate_sectors_summary).reset_index()

    print(result.columns)
    print(result.head(20))

    np.save('monthly_sectors_prices', result)
...
if __name__ == "__main__":
    ...
    get_list_of_eco_zone_and_naics(ProdConnectionString)
    ...
    monthly_prices[['pt', 'npt', 'ptvar', 'nptvar',
                    'pptvar', 'npptvar', 'rc', 'nrc', 'rcvar', 'nrcvar']] = monthly_prices[
        ['pt', 'npt', 'ptvar', 'nptvar',
    ...
    monthly_prices[['eco zone', 'naics', 'gvkey', 'isin', 'exchg', 'curr']] = monthly_prices[
        ['eco zone', 'naics', 'gvkey', 'isin', 'exchg', 'curr']].astype(str)
    ...
    for i in range(0, len(tab_of_gvkey), 16 * chunk_size):
        ...
        for j in range(0, len(chunk_gvkey), chunk_size):
            tup += monthly_stocks_price(value='', data_table=monthly_prices[
                monthly_prices['gvkey'].isin(chunk_gvkey[j:j + chunk_size])]),
    ...
    data = np.load('monthly_prices_adj.npy')
